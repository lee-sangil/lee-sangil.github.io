...
> 이전 글에서는 [레이 마칭]({% post_url 2025-04-29-Ray-marching %})(ray marching) 기법으로 불투명한 물체를 렌더링하는 방법을 설명했다. 그러나 ray가 물체의 경계에서 전진을 멈추기 때문에, 반투명하거나 투명한 물체는 제대로 렌더링할 수 없었다. 이 글에서는 기본적인 광학 물리를 도입하여 유리와 같은 물체를 렌더링하기 위한 레이 마칭 기법을 깊게 다룬다. 이 기법은 볼륨 렌더링(volume rendering) 또는 participating media rendering이라고도 한다.
