...
## Flat
Flat 보간법은 값들 사이를 채우는 가장 간단한 방법이다. 이 방법에서 두 점 사이의 값은 다음 데이터 점에 도달할 때까지 일정하게 유지된다. 예를 들어 GLSL에서 flat 보간을 사용하는 [노이즈 함수]({% post_url 2024-07-14-Noise-functions %})는 다음과 같이 작성할 수 있다.
```glsl
