...
> Let's return to creating a realistic Earth using Three.js. Unlike the previous Earth of [^PBR], we are going to render Earth using shader material. Firstly, we will describe the day and night with two different textures, since there are city lights at night. Secondly, I'll make an effect for mountain shadow and ocean reflection to make texture more realistic. Also, I'll depict the flowing cloud and its shadow. Lastly, a high-quality eclipse shadow will be cast on the surface of Earth.

## Day and night
In the previous article, [^PBR], the day and night of Earth are generated by the brightness only. It would be fine if there is no civilization. However, at present, the city lights at night are so bright enough to cause light pollution. To describe the city lights, we use a night texture, `u_nightTexture`. In Three.js, a texture map can be passed to a shader using uniform variable. Refer to [^shadermaterial]. 
```js
...
```
Seen from above, `mixAmountTexture` not only converts value from \[+1, -1\] to \[1, 0\], but also increases contrast through an exponential function for natural shading. Then, `mix()` function mixes the night color and day color. These basic functions are briefly explained in [^shaderpattern]. 
...
As mentioned in [^PBR], a single RGB value of a normal map is defined as below: 
...
// const material_earth = new THREE.MeshPhongMaterial({specular: new THREE.Color(0xFFFFFF), shininess: 5});
...
const material_earth = new THREE.ShaderMaterial({
    uniforms: {
        u_dayTexture: { value: new THREE.TextureLoader().load( PATH_TO_DAY_IMAGE ) },
        u_nightTexture: { value: new THREE.TextureLoader().load( PATH_TO_NIGHT_IMAGE ) },
        u_normalTexture: { value: new THREE.TextureLoader().load( PATH_TO_NORMAL_IMAGE ) },
        u_specTexture: { value: new THREE.TextureLoader().load( PATH_TO_SPECULAR_IMAGE ) },
        u_cloudTexture: { value: new THREE.TextureLoader().load( PATH_TO_CLOUD_IMAGE )},
