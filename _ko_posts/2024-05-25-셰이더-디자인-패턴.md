...
## 노이즈 (Noise)
서로 무작위적으로 독립적인 값들의 순열을 random이라고 부른다면, noise는 random 값들 사이를 보간하여 이웃끼리 비슷한 값을 가지는 순열을 의미한다. 따라서 [noise 함수]({% post_url 2024-07-14-Noise-functions %})는 연속성을 가지는 반면, random 함수는 불연속성을 보인다. 아래는 random과 noise의 예시다.
