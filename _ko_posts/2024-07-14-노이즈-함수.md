...
## Value 노이즈
Value 노이즈는 각 정점마다 네 개의 난수 값을 사용하는 가장 간단한 노이즈 함수다. 여기[^shaderpattern]에서 언급했듯이, GLSL은 진짜 난수 함수를 지원하지 않고 유사 난수 함수만 지원한다. 따라서 한 정점은 인덱싱, 즉 `random(x + vec2(1., 0.))`을 통해 인접한 정점의 난수 값을 읽을 수 있다. 덕분에 네 정점의 값을 보간하여 노이즈 값을 생성할 수 있다. 경계에서 노이즈 값을 부드럽게 만들기 위해 `smoothstep()`이나 커스텀 타이밍 함수를 사용할 수 있다. [보간 방법]({% post_url 2024-09-21-Interpolation-methods %})은 다음 글에서 자세히 다룬다. 
