...
결과적으로 위 시스템의 장면 그래프(scene graph)는 아래와 같다. 위성에 따라 그 객체는 모(母) 천체의 적도면이나 궤도면에 추가될 수 있다. 정확히, 장면 그래프는 Object3D, Mesh, Geometry, Material, Texture 간의 관계도 포함한다. 하지만 여기서는 Object3D만 표시하였다.
